...
void main(){
  ...
  cloudNoise = 2.0 * clamp(snoise(0.02*position + (time/10.0) , gradient), 0.0, 20.0) * cloudDensity;
  ...
  float cloudAltitude = 1.0;

  vec3 newRadius = position * vec3 (planetRadiusWater, planetRadiusWater, planetRadiusWater);
  vec3 newPosition = newRadius + normal * vec3(cloudAltitude);

  newPosition += normal*vec3(surfaceNoise);
